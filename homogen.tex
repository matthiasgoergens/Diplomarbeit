...
only a few companies and of those DB controls a huge
...
\begin{defn} A timetable \(T\) is \label{cyclic} \textit{cyclic}, iff
  there exists a period \(w \in \mathbb{R}^+\), so that for each train
...
morphism \(\varphi \colon \left(V \times \mathbb{R}\right)^2 \to
\left(V \times \mathbb{R}/{\left(w \mathbb{Z}\right)}\right)^2\).
...
\todo{How fast can we find violated constraints?  I say, fast!  At least polynomial time.  Perhaps even linear?}
