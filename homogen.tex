...
\todo{Hanno schlägt vor, Beispiel (Woche = 7 Tage) und allgemeine
  ...
  sehr.  Könnte aber mit `period' funktionieren.}
...
at and departure from different stations.
...
\begin{remark}
  ...
  flow is preserved, on a graph that is embedded in a circle of time.
  \Ie{} the flow is constant at every instant in time, because every
  moment is a cut \todo{Ist cut richtig?  Ist ja kein S-T-Schnitt,
    sondern das Auftrennen eines Tori.} in the graph.

  This allocation concentrates costs on matches spanning the period
  jump and on the deadhead trips, leaving most matches without
...
give different trips on the same weekday different coördinates.  One
...
`next' in the cycle.  (This is called \textit{abweichende Wenden} in German.
The normal continuation on the next virtual day are called \textit{logische
Wenden}.)
...
formulation in a table, then there will be empty diagonals in this case.)
