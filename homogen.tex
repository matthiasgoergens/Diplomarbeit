...
\todo{Survey of literature?  Citeseer and Arxive and Hanno.}
...
\redo{unterschied zwischen fahrplan und umlaufplan im abstract ausarbeiten.}
...
and maintenances shops.  The rail network of connected stations will
be modelled as a directed graph \(G = (V,A)\), where \(A \subseteq V
\to V\).  A \label{trip} \textit{trip} starting in one station \(v \in
V\) and ending in another station \(w \in V\) can be characterized by
a tuple \((v, t_v, w, t_w) \in \left(V \times \rationals\right)^2 \),
...
traditional way to display and visualize timetables and schedules.
This approach, called the block formulation, naturally leads to
homogeneity.
...
\todo{Talk about tori or (products of) cyclic groups?}
...
\todo{How fast can we find violated constraints?  I say, fast!  At
  least in polynomial time.  Perhaps even linear?  Linear would be a
  nice result.}
...
Aim for reduction of NP-complete problem.  Knapsack problem may also be possible?}
