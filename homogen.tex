...
% Define the set of matchings as:
...
Equation \ref{matchBedingung} holds for all \(r \in T\) and \(T\) is already finite.
We declared \(\Delta_S\) to map into \(R^+_0 \cup \{\infty\}\), but we can restrict the image even further:
...
\begin{equation}
  \label{onlyOneFinite}
  \sum_{z \in \left(\left\{d (s,t)\right\} \oplus w \mathbb{N}_0 \right) \cap [0,M] } \delta(r, q, z) \leq 1
\end{equation}
